...
public class CoinbaseExchangeImpl implements CoinbaseExchange {
    ...
    public Fill[] getFills() throws IOException, InvalidKeyException, NoSuchAlgorithmException, CloneNotSupportedException {    
        ...
        String json = generateGetRequestJSON(endpoint);
        Fill[] fills = gson.fromJson(json, Fill[].class);
    ...
    public Stats getStats(Product product) throws ClientProtocolException, IOException
    {
    	...
    	String endpoint = "/products/"+product.getId()+"/stats";
        HttpGet getRequest = new HttpGet(cbURL + endpoint);
        Authentication.setNonAuthenticationHeaders(getRequest);
        CloseableHttpResponse response = HttpClients.createDefault().execute(getRequest);
        BufferedReader br = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
    ...
    public OrderBook getOrderBook(Product product,int level) throws IOException
    {
    	...
    	String endpoint = "/products/"+product.getId()+"/book?level="+level;
        HttpGet getRequest = new HttpGet(cbURL + endpoint);
        Authentication.setNonAuthenticationHeaders(getRequest);
        CloseableHttpResponse response = HttpClients.createDefault().execute(getRequest);
        BufferedReader br = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
        String   json = processStream(br);
        Gson gson = new Gson();
        OrderBook orderbook = gson.fromJson(json, OrderBook.class);
		return orderbook;
